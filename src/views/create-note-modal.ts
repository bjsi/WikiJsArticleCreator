import { ButtonComponent, normalizePath, TextComponent, ToggleComponent } from "obsidian";
...
export class CreateNoteModal extends ModalBase {
  ...
  private publishedToggle: ToggleComponent;
	...
	async onOpen() {
		...
		this.descriptionText = new TextComponent(contentEl)
			.setPlaceholder("Describe the content")
		contentEl.createEl("br");
		...
		this.tagsText = new TextComponent(contentEl)
			.setPlaceholder("Example: tag1,tag2")
		contentEl.createEl("br")

		//
		// Published
		contentEl.appendText("Published: ");
		this.publishedToggle = new ToggleComponent(contentEl)
			.setValue(false)
		contentEl.createEl("br")

		new ButtonComponent(contentEl)
			.setButtonText("Create")
			.onClick(async () => { await this.add(false); this.close(); });
	...
	formatYamlMetadata() {
		...
		const tags = this.tagsText.getValue()
			.split(",")
			.map(s => s.trim());
		return `---
...
editor: undefined
dateCreated: ${date}
aliases: ["${this.titleText.getValue()}", "${this.titleText.getValue().toLowerCase()}"]
---`
	}

	updateNotePath(value: string) {
		const name = value.replace(/\s+/g, '');
		this.pathText.setValue(normalizePath([this.folder, sanitize(name)].join('/')));
	}
